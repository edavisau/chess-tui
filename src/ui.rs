...
impl Layout {
    fn new(size: Rect) -> Self {
        ...
        let horizontal_chunks = tui::layout::Layout::default()
            ...
            .constraints([Constraint::Min(12), Constraint::Length(22)].as_ref())
            .margin(0)
    ...
    if size.width < MIN_WIDTH {
        app.terminal.draw(|f| {
            draw_too_small(
                f,
                size,
                "Width of the screen is too small. Please make it wider.",
            )
        })?;
        ...
        app.terminal.draw(|f| {
            draw_too_small(
                f,
                size,
                "Height of the screen is too small. Please make it taller.",
            )
        })?;
        ...
        app.terminal.draw(|f| {
            ...
            draw_command_block(
                f,
                layout.command,
                app.game.get_current_colour().to_string(),
                &app.input,
            );
        })?;
    }
...
fn center_inside_rect(rect: Rect, width: u16, height: u16) -> Rect {
    ...
    Rect::new(rect.x + padding_x, rect.y + padding_y, width, height)
}
    ...
    B: Backend,
{
    ...
    let p = Paragraph::new(format!(
        "Enter a command (press ? for help)\n{} > {}",
        colour,
        input.as_str()
    ))
    .block(block);
    ...
    B: Backend,
{
    ...
    for i in 0..8 {
        for j in 0..8 {
            let tile = Rect::new(center_rect.x + 2 + i * 2, center_rect.y + 1 + j, 2, 1);
            ...
            let block =
                Block::default().style(Style::default().bg(color).fg(tui::style::Color::Black));
            f.render_widget(block, tile)
        }
    }

    let p = Paragraph::new(board).alignment(Alignment::Left);
    ...
    B: Backend,
{
    ...
    let block = Block::default()
        .title("Status")
        .borders(Borders::ALL)
        .style(Style::default().bg(if is_err {
            tui::style::Color::Red
        } else {
            tui::style::Color::Black
        }));
    let p = Paragraph::new(status_text).block(block);
    f.render_widget(p, rect);
}
